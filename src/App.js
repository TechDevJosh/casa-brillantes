...
const Contact = ({ allInquiries }) => {
  const handleSubmit = async (e) => {
    e.preventDefault();

    const form = e.target;
    const { name, email, checkin, checkout, guests, message } = form.elements;
    ...
    if (error) {
      alert('⚠️ Something went wrong. Please try again later. ' + error.message);
    ...
    <Section id="contact" title="Location & Booking" className="bg-white">
      <div className="max-w-7xl mx-auto flex flex-col lg:flex-row gap-12 lg:gap-16 bg-stone-50 p-8 md:p-12 rounded-xl shadow-sm">
        ...
        <div className="lg:w-1/2">
          ...
          <div className="mt-8">
            <h3 className="text-2xl font-bold text-stone-800 mb-4">Availability Calendar</h3>
            <BookingCalendar allInquiries={allInquiries} isPublicView={true} />
          </div>
        </div>
      </div>
    </Section>
...
function Home() {
  ...
  useEffect(() => {
    const fetchInquiries = async () => {
      ...
      if (error) {
        console.error('Error fetching inquiries for homepage:', error);
      } else {
  ...
  return (
    <>
      ...
      <Reviews />
      <Rates />
      <Contact allInquiries={allInquiries} />
    </>
  );
}
